...
namespace DES_encryption
{
    public static class DES
    {
        ...
        public static byte[] EncryptCBC(string message, string key, byte[] iv)
        {
            var messageBytes = Encoding.Default.GetBytes(message);
            return DesCbc(messageBytes, GetKeyBytes(key), iv, true);
        }
        ...
        public static string DecryptCBC(ReadOnlySpan<byte> encryptedMessage, string key, byte[] iv)
        {
            var decryptedMessage = DesCbc(encryptedMessage, GetKeyBytes(key), iv, false);
            return Encoding.Default.GetString(decryptedMessage);
        }
        ...
        private static byte[] DesCbc(ReadOnlySpan<byte> originalMessage, byte[] keyBytes, byte[] iv, bool encrypt)
        {
            var messageBytes = EnlargeMessage(originalMessage);
            var numOfParts = messageBytes.Length / 8;
            ...
            for (var i = 0; i < numOfParts; i++)
            {
                ...
                if (encrypt)
                {
                    messagePart = XorByteArrays(messagePart.ToArray(), previousBlock);
                    FeistelCypher(messagePart, subKeys, encrypt);

                    previousBlock = messagePart.ToArray();
                }
                else
                {
                    var tempBlock = messagePart.ToArray();
                    FeistelCypher(messagePart, subKeys, encrypt);

                    messagePart = XorByteArrays(messagePart.ToArray(), previousBlock);
                    previousBlock = tempBlock;
                }
                Buffer.BlockCopy(messagePart.ToArray(), 0, messageBytes, i * 8, 8);
            }
